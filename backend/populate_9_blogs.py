...
BRAIN_DIR = "/home/rushal/.gemini/antigravity/brain/dff32501-c4fc-498f-a6d5-288f8991661a"
...
# Admin credentials
ADMIN_PASSWORD = "studioravya2026"

def get_admin_token():
    req_data = json.dumps({"password": ADMIN_PASSWORD}).encode("utf-8")
...
def upload_image_via_api(token, file_path):
    import mimetypes
...
def main():
    print("[1/3] Authenticating with Studio Ravya Admin API...")
    token = get_admin_token()
    print(f"      Authenticated successfully. Admin token acquired.")

    db = SessionLocal()
    print("[2/3] Processing images and seeding 9 editorial articles...")

    for idx, art in enumerate(ARTICLES, 1):
        slug = art["slug"]
        img_filename = art["image_file"]
        img_path = os.path.join(BRAIN_DIR, img_filename)

        print(f"      ({idx}/9) Processing: '{art['title']}'")

        uploaded_url = None
        if os.path.exists(img_path):
            try:
                uploaded_url = upload_image_via_api(token, img_path)
                print(f"            Uploaded via Admin API -> {uploaded_url}")
            except Exception as e:
                print(f"            Upload API error: {e}")
                # Fallback to local copy in uploads
                dest = os.path.join(PROJECT_ROOT, "backend", "uploads", img_filename)
                import shutil
                shutil.copy(img_path, dest)
                uploaded_url = f"/uploads/{img_filename}"
        else:
            uploaded_url = "/images/showcase-saas-platform.png"

        # Also copy image to public/images for backup static resolution
        static_dest = os.path.join(PROJECT_ROOT, "public", "images", f"{slug}.jpg")
        if os.path.exists(img_path):
            import shutil
            shutil.copy(img_path, static_dest)
        ...
        if existing:
            ...
            existing.cover_image = uploaded_url
            existing.read_time_minutes = art["read_time_minutes"]
            existing.status = "published"
            existing.published_at = pub_date
            existing.meta_title = art["meta_title"]
            existing.meta_description = art["meta_description"]
        else:
            new_post = Post(
                slug=slug,
                title=art["title"],
        ...
        # Write markdown file to src/content/blog/<slug>.md
        md_path = os.path.join(CONTENT_DIR, f"{slug}.md")
        frontmatter = f"""---
title: "{art['title'].replace('"', '\\"')}"
description: "{art['excerpt'].replace('"', '\\"')}"
date: "{pub_date.strftime('%B %d, %Y')}"
category: "{art['category']}"
readTime: "{art['read_time_minutes']} min read"
coverImage: "{uploaded_url}"
featured: {str(idx == 1).lower()}
---
...
"""
    ...
    db.close()
    print("[3/3] Successfully populated all 9 articles across SQLite DB and Astro Collections.")
